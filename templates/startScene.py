...
class ListItem(RecycleDataViewBehavior, GridLayout):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        self.index = index
        self.height = (
            self.lbl_text.height
            if self.lbl_text.height < 100
            else self.lbl_text.font_size
        )
    ...
    def on_touch_down(self, touch):
        ...
        if self.collide_point(*touch.pos) and self.selectable:
            if self.func and self.root:
                self.root.execute_func(self.func, self.params)
...
class GameView(Screen):
    ...
    # lbl_title: ObjectProperty(CenteredLabel)
    # lbl_authors: ObjectProperty(CenteredLabel)
    # lbl_release: ObjectProperty(CenteredLabel)
    # lbl_tags: ObjectProperty(CenteredLabel)
    # lbl_desc: ObjectProperty(CenteredLabel)
...
class SideMenu(BoxLayout):
    ...
    def prepare_link(self, name):
        if name == "github":
            ...
            Logger.error(
                f'"{name}" has no url mapping. Check URL mapping for button names.'
            )
...
class StartScene(Screen):
    ...
    def __init__(self, story_names: Dict[str, str], idx: ListProperty, **kw):
        ...
        self.story_list.mydata = [
            {
                "text": name,
                "root": self,
                "func": "create_game_view",
                "params": {"name": name},
            }
            for name in story_names.keys()
        ]
    ...
    def create_game_view(self, **kw):
        story_name = kw["name"]
        a = self.app
        a.story_info = self.sparql.load_story_info(
            story_name, path=os.path.join(os.getcwd(), "stories")
        )
