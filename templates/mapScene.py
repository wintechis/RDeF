...
class MapScene(Screen):
    ...
    def remove_pins(self, pins: List):
        for c in self.children:
            if c not in pins and isinstance(c, Pin):
                self.remove_widget(c)

    def add_pins(self, pins: List):
        for pin in pins:
            if pin in self.children:
    ...
    def show_detail(self, pin: Pin):
        if pin.state == "normal":
            ...
            if pin is self.wd_detail.pin:
                self.remove_widget(self.wd_detail)
        else:
            new_color = [1 - x for x in pin.my_color]
            new_color[3] = 1  # alpha value
            pin.color = new_color
            self.wd_detail.update(pin)
            if self.wd_detail not in self.children:
                self.add_widget(self.wd_detail)
...
class DetailWidget(RelativeLayout):
    ...
    def update(self, pin: Pin):
        if self.pin not in [None, pin]:
            self.pin.state = "normal"
