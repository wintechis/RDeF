...
from templates.mapScene import Detail, Location
from templates.queryScene import QueryItem, QueryScene
...
class Episode(Screen):
    ...
    def __init__(
        self, uri: str, location: Location, detail: Detail, chapter_path: str, **kw
    ):
        self.name = uri
        self.uri = uri
    ...
    def load(self, g: rdflib.Graph):
        ...
        scenes_as_uri = self.sparql.execute(
            "get_scenes", g, {"_:placeholder": f"<{self.name}>"}
        )
        scenes = list(
            map(
                lambda x: [x["type"].toPython(), f"<{x['scene'].toPython()}>"],
                scenes_as_uri,
            )
        )
        for scene in scenes:
            if scene[0].endswith("TalkScene"):
                ...
                s = QueryScene(
                    query_item=QueryItem(
                        question=q["question"].toPython(),
                        markup_query=q["query"].toPython(),
                    ),
                    episode_graph=self.g,
    ...
    def get_talk(self, g, uri):
        dialogue = self.sparql.execute(
            "get_talk", g, {"_:placeholder": uri}
        )  # self.get_talk_data(g, uri)
        l = []
        for talk in dialogue:
            triples = self.sparql.execute(
                "get_statements_in_talk_item",
                g,
                {
                    "_:placeholder": uri,
                    ":_placeholder": "<" + talk["idx"].toPython() + ">",
                },
            )
            tri = []
            for triple in triples:
                ...
                if any(
                    ...
                    lbls = [
                        lbl.toPython()
                        for lbl in self.sparql.get_list_items(g, triple["labels"], l=[])
                    ]  # keywords
                    tri.append(
                        [
                            {
                            ...
                            triple["update"].toPython()
                            if "update" in triple.keys()
                            else [
                                triple[term]
                                for i, term in enumerate(
                                    ["subject", "predicate", "object"]
            ...
            s = Speaker(
                name=talk["name"].toPython(),
                depiction=ResourceManager.get_resource_path(
                    self.chapter_path, talk["img"].toPython()
                ),
            )
        ...
        return TalkInfo(
            dialogue=l,
            background=ResourceManager.get_resource_path(
                self.chapter_path, talk["background"]
            ),
        )

    def get_namespaces(self, triple: dict) -> List[Tuple[str]]:
        if "update" in triple.keys():
            update = triple["update"].toPython()
        elif "namespaces" in triple.keys():
            update = triple["namespaces"].toPython()
        else:
            return []
