...
from myWidgets.myLabels import DragLabel, NormalLabel, PlaceholderLabel
...
class FileViewerView(BoxLayout):
    ...
    def activate_tab(
        self, header: Union[str, TabbedPanelHeader, TabbedPanelItem]
    ) -> None:
    ...
    def __activate_tab_by_str(self, header: str) -> None:
        for tab in self.viewer.tab_list:
            if tab.text == header:
                self.viewer.switch_to(tab)

    def __activate_tab_by_obj(
        self, header: Union[TabbedPanelHeader, TabbedPanelItem]
    ) -> None:
        for tab in self.viewer.tab_list:
            if tab == header:
                self.viewer.switch_to(tab)
...
class QuerySpace(GridLayout):
    ...
    def fill_query_space(self, lst: List, is_drag: bool):
        ...
        for i, word in enumerate(lst):
            ...
            if "\n" in word and i != 0:
                new_line = True
                stack_line = self.add_new_stackline()
            lst_indent, turtle_indent = self.calculate_indents(
                word,
                lst[i - 1],
                lst[i + 1] if len(lst) > i + 1 else "\n",
                lst_indent,
                turtle_indent,
            )

            if new_line:
                self.add_indents(stack_line, lst_indent, turtle_indent)
            self.add_label(stack_line, word, is_drag)

    def calculate_indents(
        self,
        cur_word: str,
        prior_word: str,
        next_word: str,
        bracket_indent: List[str],
        turtle_indent: str,
    ) -> Tuple[List[str], str]:
    ...
    def add_indents(
        self, line: QueryLine, lst_indent: List[str], turtle_indent: str
    ) -> None:
        ...
        if turtle_indent:
            self.add_label(line, turtle_indent, True)

    def add_label(self, line: QueryLine, word: str, is_drag: bool) -> None:
        if word[0] == "$":
            # if not is_drag, then is_blank instead
            line.add_widget(
                PlaceholderLabel(text="  " * self.max_len)
            ) if is_drag else line.add_widget(QueryBlank(char_limit=self.max_len))
        else:
    ...
    def reset(self):
        ...
        self.lst_query = [
            s for s in new_query.split(" ") if s
        ]  # remove spaces, indendation is handled by add_indents
...
class QueryBlank(TextInput):
    ...
    def keyboard_on_key_down(self, window, keycode: Tuple[int, str], text, modifiers):
        if keycode[1] == "shift":
            QueryBlank.hold_shift = True
        if keycode[1] == "tab":
            if QueryBlank.hold_shift:
...
class DragLabelStart(StackLayout):
    ...
    def reset(self):
        ...
        for word in sorted(self.draggables):
            self.drag_labels.append(
                DragLabel(text=word, drag_area=self.parent.parent, start_area=self)
            )
...
class QueryScene(Screen):
    ...
    def __init__(
        self, query_item: QueryItem, episode_graph: Graph, chapter_path: str, **kwargs
    ):
        super().__init__(**kwargs)
    ...
    def on_enter(self, **kw):
        ...
        self.chapters_graph = initalize_graph(keep_prefixes=False)
        ...
        for file_name in files:
            self.chapters_graph.parse(
                os.path.split(self.chapter_path)[0] + f"/db/{file_name}.ttl"
            )
        ...
        nm = {
            n.toPython(): p
            for p, n in self.chapters_graph.namespace_manager.namespaces()
        } | {n.toPython(): p for p, n in self.g.namespace_manager.namespaces()}
        self.display_result(
            input=self.rst_solution, output=self.upper_view.contents["target"], nm=nm
        )
        self.lower_view.contents["target"].text = self.upper_view.contents[
            "target"
        ].text
    ...
    def load_file(self, instance, file_name):
        if file_name == "current":
            ...
            instance.parent.parent.graph = g.parse(
                os.path.split(self.chapter_path)[0] + f"/db/{file_name}.ttl"
            )
    ...
    def execute_query(self, instance):
        ...
        query = (
            content.text
            if self.query_panel.query_tab.text == "Free"
            else " ".join(
                reversed(
                    [word.text for line in content.children for word in line.children]
                )
            )
        )
        ...
        try:
            ...
            self.display_result(
                input=rst,
                output=self.upper_view.contents["query"],
                nm=self.get_my_namespaces(query),
            )
    ...
    def display_result(
        self, input: rdflib.query.Result, output: TextInput, nm: Dict[str, str]
    ):
    ...
    def create_prefix_table(
        self, resultSet: List[rdflib.query.ResultRow], all_nm: Dict[str, str]
    ) -> Tuple[PrettyTable, NamespaceManager]:
        tbl_p = PrettyTable()
        tbl_p.field_names = ["PREFIX", "NAMESPACE"]
        nm = self.get_rel_namespace_manager(
            resultSet, all_nm
        )  # retrieve only relevant namespaces
    ...
    def create_result_table(
        self, resultSet: List[rdflib.query.ResultRow], nm: NamespaceManager
    ) -> Tuple[PrettyTable, List[str]]:
        d_header = {}
        tbl_rst = PrettyTable()
        # get all values
    ...
    def create_meta_table(
        self, resultSet: List[rdflib.query.ResultRow], header: List[str]
    ) -> PrettyTable:
    ...
    def get_rel_namespace_manager(
        self, result: List[rdflib.query.ResultRow], nm_dict: Dict[str, str]
    ) -> NamespaceManager:
